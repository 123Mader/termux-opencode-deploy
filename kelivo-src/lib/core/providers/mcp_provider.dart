...
class McpProvider extends ChangeNotifier {
  ...
  Future<void> _load() async {
    ...
    // Ensure built-in DeXRay AI reverse-engineering server is present by default
    final dexray = _builtinDexrayServerIfMissing();
    if (dexray != null) {
      final next = <McpServerConfig>[..._servers, dexray];
      await _persistServers(next);
      _servers = next;
    }
    // initialize statuses
  ...
  /// Ensure built-in DeXRay AI (APK reverse-engineering) MCP server is present.
  /// Points at the local DeXRay MCP server (http://127.0.0.1:8791/mcp).
  McpServerConfig? _builtinDexrayServerIfMissing() {
    final exists = _servers.any(
      (s) =>
          s.name == 'DeXRay AI' ||
          s.id == 'dexray_ai' ||
